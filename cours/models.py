    ...
    def save(self, *args, **kwargs):
        ...
        if not self.image_module:
            if not self.image_module:
                ...
                #font = ImageFont.truetype('/static/cours/SpaceMono-Bold.ttf', 80)
                draw = ImageDraw.Draw(default_image)
                draw.text((60, 60), "Hello Mamadou", fill="blue")

                draw.text((10, 10), f"{self.name}", fill='black')
...
class Note(models.Model):  #Note par matiere
    ...
    def save(self, *args, **kwargs):
        fake = Faker()
        help = fake.bothify("###??")
        self.identifiant = f"{self.etudiant}~{self.module.name}~{self.session}~{help}"
        mon_mois = datetime.now().month
        if 7 <= mon_mois <= 9:
            self.session = f"Automne {datetime.now().year}"
        if mon_mois == 12 or mon_mois == 1:
            self.session = f"Hiver {datetime.now().year}"
        if mon_mois == 4 or mon_mois == 5:
            self.session = f"Ete {datetime.now().year}"
...
class Choix_Cours(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        self.identifiant = f"{self.identifiant}~{self.user}"
        super().save(*args, **kwargs)
...
class Payer(models.Model):
    ...
    session = models.CharField(max_length=100, default="Automne 2024")
...
class Historique(models.Model):
    ...
    backup = models.JSONField(default=dict)

    def save(self, *args, **kwargs):
        fake = Faker()
        now = datetime.now()
        dt = now.strftime('%d-%m-%Y')
        self.identifiant = f"{dt}~{self.interesser}~{fake.bothify('??-##')}"
        super().save(*args, **kwargs)
    class Meta:
...
class Create_groupe(models.Model):
    ...
    file_prof = models.CharField(max_length=500, blank=True)
    session = models.CharField(max_length=90, blank=True)
    #document_file = models.FileField(blank=True)
    #recuperation = models.JSONField(default=list, blank=True)
...
class Stocke_media_file(models.Model):
    identifiant_file = models.CharField(max_length=70, blank=True)
    type_file = models.CharField(max_length=255)

    mon_fichier = models.FileField(upload_to='tp', blank=False)
