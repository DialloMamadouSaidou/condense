                ...
                if len(mykey) != 0:
                    ...
                    try:
                        if choix in ["charge_cours", "etudiant"] and other_choice is None:
                            ...
                            new_user = Profile.objects.create(user=element, choices=choix, domaine_programme=other_choice)
...
def modification_password(request):
    if request.method == "POST" and request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        try:
            ...
            mon_email = reverse('user:recupere',    kwargs={'email': email})
...
def recupere_code(request, email):
    if request.method == "POST" and request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        try:
            ...
            if my_user.code_secret == code:
                mon_url = reverse('user:receive', kwargs={'ids': email})
                return JsonResponse(
                    {'success': True, 'message': "Code validé avec succès!", 'redirect_url': mon_url})

            else:
                return JsonResponse({'success': False})

        except MyUser.DoesNotExist:
            print("Cet utilisateur na pas de compte dabord")
...
#La fonction est à réecrire
def renvoie_code(request):
    pass
